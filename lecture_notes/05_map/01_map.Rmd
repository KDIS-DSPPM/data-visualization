...
pacman::p_load(gapminder, here, tidyverse, ggrepel, socviz)
```

## Mapping in R 

Once again, the most of the code and instructions are adapted from Kieran Healy's data visualization book and his code use in the RStudio conference: https://github.com/rstudio-conf-2020/dataviz/blob/master/07_maps.Rmd 

```{r}
election %>% select(state, total_vote, r_points, pct_trump, party, census) %>% 
  slice_max(order_by = pct_trump, n = 5)
```
...
p + geom_polygon(color = "gray90", size = 0.1) +
    coord_map(projection = "albers", lat0 = 39, lat1 = 45) + # curved a little bit: lat 0, lat1 conventioanl values for the US map
    guides(fill = FALSE)
```
